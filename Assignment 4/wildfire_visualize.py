...
def save_vtk(forest, day):
    """Saves the forest grid state as a VTK file using pyvtk."""
    
    # Convert the forest state to a 1D array (flattened)
    ...
    vtk_data = pyvtk.VtkData(
        ...
        pyvtk.PointData(
            pyvtk.Scalars(forest_flat, name="State")  # Forest state as a scalar field
        )
    )
    
    # Generate filename with zero-padded day number (e.g., forest_day_01.vtk)
    filename = f"forest_day_{day:02d}.vtk"
